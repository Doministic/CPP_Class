...
static std::pair<int, int> setDifficulty(difficulty level)
{
...
static int printDifficulty(int& level)
{
	difficulty diffLevel = static_cast<difficulty>(setDifficultyFromUserInput(level));
...
{
	{
		...
		std::pair<int, int> difficultyLevel = setDifficulty(Normal); // Default difficulty level
		...
		int numberToGuess = setDifficulty(level).first;
		int attempts = setDifficulty(level).second;

		std::cout << "The number to guess is set. You have " << attempts << " attempts to guess it.\n";

		int userGuess;
		//TODO: FINISH the guessing game. 
	}
